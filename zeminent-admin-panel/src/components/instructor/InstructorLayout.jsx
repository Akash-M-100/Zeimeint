...
// Shell for the whole instructor panel: auth guard + fixed sidebar + sticky
// topbar + content. Anyone who isn't an instructor is sent to the login screen.
export default function InstructorLayout({ children }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { user, isInstructor, hydrated } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (hydrated && !isInstructor) {
      router.replace("/login");
    }
  }, [hydrated, isInstructor, router]);
